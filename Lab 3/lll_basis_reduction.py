...
if __name__=="__main__":
    '''
    For 2D basis use n_dim = 2
    For 3D basis use n_dim = 3
    '''
    n_dim = int(input("Enter basis dimension: "))
    m = n_dim
    n = n_dim
    matrix = [[0 for j in range(n)] for i in range(m)]
    for i in range (0,m):
        print(f"Enter basis vector {i+1}: ")
        matrix[i] = list(map(int,input().strip().split()))[:n]
    delta = float(input("Enter the angle delta: "))
    res = reduction(matrix, delta)
    print("LLL-Reduced basis: ", res)